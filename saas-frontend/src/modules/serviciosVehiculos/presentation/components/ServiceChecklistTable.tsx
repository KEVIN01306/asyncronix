    ...
    <Paper sx={{ p: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h6">Checklist</Typography>
            <Box>
          {!editingAll ? (
            <Button variant="outlined" size="small" onClick={startEditAll} sx={{ mr: 1 }} disabled={savingAll || servicio.estado !== ESTADO_SERVICIO_VEHICULO.RECEPCION}>Editar checklist</Button>
          ) : (
            <>
            ...
            <TableBody>
              {servicio.checklist.map((respuesta) => {
                ...
                return (
                    ...
                    <TableCell><Button size="small" onClick={() => startEditRespuesta(respuesta)} disabled={savingAll || savingRespuesta || servicio.estado !== ESTADO_SERVICIO_VEHICULO.RECEPCION}>Editar</Button></TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
