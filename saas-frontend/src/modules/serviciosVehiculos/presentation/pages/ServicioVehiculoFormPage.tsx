    ...
    const onSubmit = async (data: ServicioFormValues) => {
        ...
        try {
            if (isEdit && params.id) {
                ...
                navigate(`/servicios-vehiculo/${params.id}/configuracion`);
            } else {
                const service = await servicioRepository.registrar(payload);
                toast.success('Servicio creado correctamente');
                navigate(`/servicios-vehiculo/${service.id}/configuracion`);
            }
        } catch (error) {
            console.error(error);
                    ...
                    {(step === 'detalles' || step === 'editar') && vehiculo && (
                        <Box>
                            <Paper sx={{ p: 3, mb: 2 }}>
                                ...
                                <Box sx={{ bgcolor: 'paper.main', p: 2, borderRadius: 1, display: 'grid', gap: 1, border: `2px dashed ${theme.palette.grey[200]}` }}>
