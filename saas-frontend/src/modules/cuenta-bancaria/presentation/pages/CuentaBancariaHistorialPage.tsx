...
import type { TransaccionHistorial } from '../../infrastructure/cuenta-bancaria.repository';
...
const CuentaBancariaHistorialPage = () => {
    ...
    const fetchHistorial = useCallback(
        async (signal: AbortSignal) => {
            ...
            setLoading(true);
            try {
                const response = await cuentaBancariaRepository.obtenerHistorial(id, limit, offset, signal);
            ...
            >
                <Stack direction={isMobile ? "column" : "row"} justifyContent="space-between" alignItems={isMobile ? "flex-start" : "center"} spacing={2}>
                    ...
                    <Box textAlign={isMobile ? "left" : "right"} mt={isMobile ? 1 : 0} width={isMobile ? "100%" : "auto"}>
                        ...
                        <Chip
                            size="small"
                            label={isIngreso ? 'Crédito' : 'Débito'}
