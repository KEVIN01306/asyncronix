...
export class CrearServicioRepuestoUseCase {
    ...
    async execute(servicioId: string, productoId: string, cantidad: number, negocio_id: string, sucursal_id: string): Promise<any> {
        try {
            const producto = await this.productoRepository.obtener(productoId, negocio_id);
            if (!producto) throw new AppError('Producto no encontrado', 'PRODUCTO_NO_ENCONTRADO', 404);
            ...
            if (!lotes || lotes.length === 0) throw new AppError('No hay lotes activos con stock para el producto', 'NO_LOTE_DISPONIBLE', 400);
