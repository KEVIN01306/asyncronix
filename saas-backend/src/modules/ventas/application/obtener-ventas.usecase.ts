...
export class ObtenerVentasUseCase {
    ...
    async execute(negocio_id: string, sucursal_id: string, pagination: Pagination): Promise<Paginated<VentaSimple>> {
        try {
            return await this.ventaRepository.listar(negocio_id, sucursal_id, pagination);
        } catch (error) {
