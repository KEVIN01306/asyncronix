...
export class ServicioController extends BaseController {
    ...
    registrarRepuesto = async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
        try {
            const { id } = req.params;
            const { negocio_id } = this.obtenerEntorno(res);
            const { sucursal_id, variante_id, codigo, cantidad } = req.body;
            ...
            const detalle = await this.crearServicioRepuestoUseCase.execute(id, variante_id, codigo, cantidad, negocio_id, sucursal_id);
    ...
    registrar = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const { id: recepcionista_id, negocio_id } = this.obtenerEntorno(res);
            const servicio = await this.registrarServicioUseCase.execute(req.body, negocio_id, recepcionista_id);
