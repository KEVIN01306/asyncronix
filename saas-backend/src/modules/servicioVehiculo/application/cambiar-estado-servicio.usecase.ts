    ...
    async execute(id: string, negocio_id: string, estado: string): Promise<ServicioDetalle> {
        try {
            ...
            try {
                ...
                if (estado === ESTADO_SERVICIO.EN_PRUEBAS) {
                    destinatarioId = servicio.recepcionista_id ?? undefined;
                    titulo = 'Servicio en pruebas';
                    cuerpo = `El servicio ${servicio.tipo_servicio?.nombre} con el modelo de la moto ${modelo} ahora está en pruebas`;
                } else if (servicio.mecanico?.id) {
                    destinatarioId = servicio.mecanico.id;
                ...
                if (destinatarioId && this.enviarNotificacionUseCase) {
                    await this.enviarNotificacionUseCase.execute(destinatarioId, titulo, cuerpo);
                }
            } catch (err) {
